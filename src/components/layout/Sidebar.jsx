  ...
  return (
    <aside className="w-[220px] bg-white border-r border-black/[0.09] flex flex-col py-5 flex-shrink-0 hidden md:flex">
      ...
      <div className="px-5 pb-6 border-b border-black/[0.09] mb-2">
        <div className="w-9 h-9 bg-[#639922] rounded-lg flex items-center justify-center mb-2.5">
      ...
      <nav className="px-3 flex-1">
        ...
        {navItems.map((item) => (
          <button
            ...
            `}
          >
            <i className={`ti ${item.icon} text-[17px] ${activePage === item.page ? "text-[#639922]" : ""}`} aria-hidden="true" />
            {item.label}
          </button>
        ))}
      </nav>
      ...
      <div className="px-5 pt-4 border-t border-black/[0.09]">
        <span className="inline-flex items-center gap-1.5 bg-[#EAF3DE] text-[#3B6D11] text-[11px] font-semibold px-2.5 py-1 rounded-full">
